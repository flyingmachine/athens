...
(defn sorted-content
  ...
  #(sort-by sort-fn (athens.db.query/all content-attribute [:content/author (:db/id %)])))
...
(defmaprules ent->topic
  ...
  (has-one :first-post
           :rules athens.db.maprules/ent->post
           :retriever :topic/first-post)
  (has-one :author
           :rules athens.db.maprules/ent->user
  ...
  (has-many :posts
            ...
            :retriever #(sort-by :post/created-at
                                 (:post/_topic %)))
  (has-many :watches
            :rules athens.db.maprules/ent->watch
...
(defmaprules ent->post
  ...
  (has-one :author
           ...
           :retriever :content/author)
  (has-one :topic
           :rules athens.db.maprules/ent->topic
...
(defmaprules ent->user
  ...
  (has-many :topics
            :rules athens.db.maprules/ent->topic
            :retriever #(athens.db.query/all :topic/title [:content/author (:db/id %)]))
  (has-many :posts
            :rules athens.db.maprules/ent->post
            :retriever (athens.db.maprules/sorted-content :post/content :post/created-at)))
